...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // 2. If not logged in, either render public routes or redirect
  if (!user) {
    if (isPublicRoute) {
  ...
  // 3. If logged in, but on a public route, redirect to home
  if (user && isPublicRout) {
      router.push('/');
      return (
          <div className="flex min-h-screen w-full items-center justify-center">
              <div className="flex items-center gap-3">
  ...
  // 4. If logged in and on a protected route, render the full app shell.
  return (
