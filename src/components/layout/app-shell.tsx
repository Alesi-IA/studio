...
import { Home, Calendar, Bot, BookOpen, MessageSquare, User, PlusCircle } from "lucide-react";
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 border-t bg-background flex items-center justify-around z-20">
        {menuItems.map((item) => (
          <Link
            ...
            href={item.href}
            className={cn(
              "flex flex-col items-center gap-1 p-2 rounded-md transition-colors text-muted-foreground hover:bg-accent",
